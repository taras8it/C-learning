 ...
 * Description: Assignment 4.1 – – Grade administration system
 * Date: 14-9-2021
 *****************************************/
#include <stdio.h>
...
double read_double_input(int i) {
    ...
    while (scan_return == 0) {
        ...
        // If no character matched the scanf mask, run the loop again
...
int main() {
    ...
    if (res == 0) {
        ...
        double avg = count_average(grades, sizeof(grades) / sizeof(grades[0]));
        printf("The average grade of the class: %.1lf.\n", avg);

        // Print the number of students who passed
        int number = count_pass(grades, sizeof(grades) / sizeof(grades[0]));
